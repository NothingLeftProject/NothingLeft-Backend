...
class UserPermissionManager:
    ...
    def get_user_permissions(self, account, cache_to_memcached=True, ask_update=False):
        ...
        if permissions_list is None or cache_to_memcached is False:
            group_name = self.mongodb_manipulator.get_document("user", account, {"userGroup": 1}, 2)[0]["userGroup"] # ATTENTION: error might be here
            if group_name is False:
                self.log.add_log("UserPermissionManager: can't find the account-" + account + " or something wrong", 3)
                return False, "user does not exists"

            permissions_list = self.mongodb_manipulator.get_document("user_group", group_name, {"_id": 2}, 2)[0]["PermissionList"]
            different_user_list = self.mongodb_manipulator.get_document("user_group", group_name, {"_id": 3}, 2)[0]["differentUsers"]

            if account in different_user_list:
                permission_difference = self.mongodb_manipulator.get_document("user_group", group_name, {"permissionDifferences": 1}, 2)[0]["permissionDifferences"][account]
                try:
    ...
    def write_user_permissions(self, account, new_permissions_list):
        ...
        group_name = self.mongodb_manipulator.get_document("user", account, query={"userGroup": 1}, mode=2)[0]["userGroup"]
        group_permissions_list = self.mongodb_manipulator.get_document("user_group", group_name, query={"permissionList": 1}, mode=2)[0]["permissionList"]
        different_list = []
        ...
        if is_different:
            different_users = self.mongodb_manipulator.get_document("user_group", group_name, query={"_id": 3}, mode=2)[0]["differentUsers"]
            ...
            different_permissions_list = self.mongodb_manipulator.get_document("user_group", group_name, query={"_id": 4}, mode=2)[0]["permissionDifferences"]
    ...
    def edit_user_permissions(self, account, permissions_to_change):
        ...
        raw_permissions_list = self.mongodb_manipulator.get_document("user", account, query={"_id": 12}, mode=2)[0]["permissionList"]
