    ...
    def auth(self):
        ...
        if "@" in account or str(int(account)) == account: # ATTENTION: error might be here
            ...
            if time_loss > 0 and time_loss < 60:
                last_login_time_stamp = self.mongodb_mainpulator.get_document("user", account, query={"_id": 13}, mode=2)["lastLoginTimeStamp"]
                ...
                if login_time_loss > 0 and login_time_loss < 3600*24:
                    ...
                    # is token same
                    real_token = self.mongodb_mainpulator.get_document("user", account, query={"_id": 7}, mode=2)["token"]
            ...
            self.log.add_log("HttpHandler: account not exists", 1)
            self.response_data["header"]["errorMsg"] = "user does not exists" # user not exists error
            return False
